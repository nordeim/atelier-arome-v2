# DEEP_PROJECT_UNDERSTANDING_DOCUMENT.md
...
**File:** `/home/project/atelier-arome/docker/docker-compose.yml`
...
postgres:
  ...
  environment:
    POSTGRES_USER: ${POSTGRES_USER:-atelier_dev_user}
    POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:-secure_dev_password}
    POSTGRES_DB: ${POSTGRES_DB:-atelier_dev_db}
  ports:
    - "${POSTGRES_PORT:-5432}:5432"
  volumes:
    - postgres_data:/var/lib/postgresql/data
  healthcheck:
    test: ["CMD-SHELL", "pg_isready -U ${POSTGRES_USER:-atelier} -d ${POSTGRES_DB:-atelier_development}"]
```

**Key Details:**
- **Image:** `postgres:16-alpine` (lightweight, production-grade)
- **Container Name:** `atelier_db`
- **Default User:** `atelier_dev_user`
- **Default Password:** `secure_dev_password` (dev only)
- **Default Database:** `atelier_dev_db`
- **Port Mapping:** `0.0.0.0:5432` → `container:5432`
- **Volume Persistence:** `postgres_data` → `/var/lib/postgresql/data`
- **Health Check:** `pg_isready` command to verify database readiness
- **Connection URL:** `postgres://atelier_dev_user:secure_dev_password@db:5432/atelier_dev_db`
...
**Key Details:**
- **Image:** `redis:7.4-alpine` (lightweight, production-grade)
- **Container Name:** `atelier_redis`
- **Port Mapping:** `0.0.0.0:6379` → `container:6379`
- **Volume Persistence:** `redis_data` → `/data`
- **No Password:** Development mode (no authentication required)
...
mailhog:
  ...
  container_name: atelier_mailhog
  ports:
...
**Key Details:**
- **Image:** `mailhog/mailhog:latest`
- **Container Name:** `atelier_mailhog`
- **SMTP Port:** `1025` (email submission)
- **Web UI Port:** `8025` (email viewing in browser)
- **Access:** http://localhost:8025 (view sent emails)

### Container Status (Verified)

| Container | Image | Status | Uptime | Ports | Purpose |
|-----------|--------|--------|--------|--------|---------|
| **atelier_db** | postgres:16-alpine | ✅ Up (healthy) | 22 hours | 0.0.0.0:5432→5432, [::]:5432→5432 | Primary database |
| **atelier_redis** | redis:7.4-alpine | ✅ Up (healthy) | 22 hours | 0.0.0.0:6379→6379, [::]:6379→6379 | Cache, session, queue |
| **atelier_mailhog** | mailhog/mailhog:latest | ✅ Up | 22 hours | 1025 (SMTP), 8025 (Web UI) | Email testing |
...
DB_HOST=127.0.0.1
DB_PORT=5432
DB_DATABASE=atelier_dev_db
DB_USERNAME=atelier_dev_user
DB_PASSWORD=secure_dev_password
```

**Docker Environment Variables (Defaults):**
```yaml
POSTGRES_USER=atelier_dev_user
POSTGRES_PASSWORD=secure_dev_password
POSTGRES_DB=atelier_dev_db
POSTGRES_PORT=5432
```

**Connection String:**
```
postgres://atelier_dev_user:secure_dev_password@127.0.0.1:5432/atelier_dev_db
```

### Database Schema Verification (Validated via psql)

**Connection Command:**
```bash
docker exec -it atelier_db psql -U atelier_dev_user -d atelier_dev_db -c "\dt"
```
...
--------+------------------------+-------+------------------
public | addresses               | table | atelier_dev_user
public | cart_items              | table | atelier_dev_user
public | carts                   | table | atelier_dev_user
public | categories              | table | atelier_dev_user
public | coupon_usages           | table | atelier_dev_user
public | coupons                 | table | atelier_dev_user
public | inventories             | table | atelier_dev_user
public | inventory_movements     | table | atelier_dev_user
public | migrations              | table | atelier_dev_user
public | newsletter_subscribers  | table | atelier_dev_user
public | order_items             | table | atelier_dev_user
public | orders                  | table | atelier_dev_user
public | password_reset_tokens    | table | atelier_dev_user
public | payments                | table | atelier_dev_user
public | product_images          | table | atelier_dev_user
public | product_tag            | table | atelier_dev_user
public | product_variants        | table | atelier_dev_user
public | products               | table | atelier_dev_user
public | reviews                 | table | atelier_dev_user
public | sessions                | table | atelier_dev_user
public | settings                | table | atelier_dev_user
public | tags                    | table | atelier_dev_user
public | testimonials            | table | atelier_dev_user
public | users                   | table | atelier_dev_user
public | wishlist_items          | table | atelier_dev_user
public | wishlists               | table | atelier_dev_user
```
...
------------------+------------------------------------------------------------
 atelier_dev_user | Superuser, Create role, Create DB, Replication, Bypass RLS <system-reminder>
```

**Note:** The `atelier_dev_user` role has superuser privileges, which is appropriate for development but should be restricted in production.
...
**Redis Usage:**
- **Cache:** Application cache (config, routes, computed data)
- **Sessions:** User session storage
- **Queue:** Job queue for background tasks (email, PDF generation)
- **Health Check:** `docker exec atelier_redis redis-cli ping` → `PONG`

### Mailhog Usage (Validated)
...
**Accessing Mailhog:**
- **SMTP:** smtp://127.0.0.1:1025 (used by Laravel)
- **Web UI:** http://localhost:8025 (view sent emails)
- **Purpose:** Capture all emails sent during development without external SMTP

### Volume Persistence (Docker Managed)
...
**Benefits:**
- **Data Persistence:** Database survives container restarts/recreations
- **Backup Ready:** Volumes can be backed up easily
- **Development Safety:** No data loss when rebuilding containers
...
2. **Products & Categories (7 tables):**
   ...
   - ✅ `inventories` (table created, model not in Models/)
   - ✅ `inventory_movements` (table created, model not in Models/)
...
5. **Implement API Resources** (3 hours)
...
6. **Create Base Controller** (2 hours)
   - Common CRUD methods (index, store, show, update, destroy)
   - Standardized error responses
   - Pagination support

7. **Implement Authentication** (4 hours)
...
8. **Complete Design System** (5 hours)
...
**Observation:** The atomic component architecture (60-150 lines per component) has proven effective for maintaining code quality and preventing monolithic file issues encountered earlier.
...
**Observation:** The Illuminated Manuscript design system (custom colors, fonts, spacing) must be consistently applied across all components to maintain distinctive aesthetic.
...
1. ✅ **Solid Database Design:** 26 tables with UUID primary keys, soft deletes, snapshot pattern
...
**Analysis Methodology:** Documentation review + codebase validation + Docker infrastructure deep dive  
**Confidence Level:** **High** (validated against actual implementation)

---

*End of Deep Project Understanding Document*
